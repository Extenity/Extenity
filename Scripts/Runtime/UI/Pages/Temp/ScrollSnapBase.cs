...
namespace Extenity.UIToolbox
{
	...
	public class ScrollSnapBase : MonoBehaviour, IBeginDragHandler, IDragHandler
	{
		...
		void Awake()
		{
			...
			// HACK: Disabled to allow a template child item in a dynamically created UI.
			// if (ChildObjects != null && ChildObjects.Length > 0)
			// {
			//     if (_screensContainer.transform.childCount > 0)
			//     {
			//         Debug.LogError($"{nameof(ScrollSnapBase)} Content has children, this is not supported when using managed Child Objects\n Either remove the ScrollRect Content children or clear the ChildObjects array");
			//         return;
			//     }
		...
		{
			return _isVertical ? -((pos.y - _scrollStartPosition) / _childSize) == -(int)Math.Round((pos.y - _scrollStartPosition) / _childSize) : -((pos.x - _scrollStartPosition) / _childSize) == -(int)Math.Round((pos.x - _scrollStartPosition) / _childSize);
		}
