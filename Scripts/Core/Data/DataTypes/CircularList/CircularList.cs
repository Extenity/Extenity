...
namespace Extenity.DataToolbox
{
	...
	public class CircularList<T> : ICollection//, ICollection<T>
	{
		...
		public int Capacity
		{
			...
			set
			{
				...
				if (value > 0)
				{
					...
					if (count > 0)
					{
						// Copy old items to new list. Rearrange items while doing that.
						if (CyclicTailIndex == 0)
						{
		...
		public T HeadingItem
		{
			get
			{
				if (CyclicHeadIndex < 0)
					throw new ArgumentOutOfRangeException("HeadingItem", "Tried to get heading item while the list is empty.");
		...
		public T TailingItem
		{
			...
			{
				if (CyclicTailIndex < 0)
					throw new ArgumentOutOfRangeException("TailingItem", "Tried to get tailing item while the list is empty.");
		...
		{
			if (IsEmpty)
				throw new Exception("Tried to remove heading item while the list is empty.");
		...
		{
			if (IsEmpty)
				throw new Exception("Tried to remove tailing item while the list is empty.");
		...
		public struct Enumerator : IEnumerator<T>
		{
			...
			public bool MoveNext()
			{
				...
				if (Remaining > 0 && Version == localList.Version)
				{
					if (Index == Capacity)
						Index = 0;
					_Current = localList.Items[Index];
					Index++;
					Remaining--;
					return true;
				}
		...
		public CircularList<TOutput> ConvertAll<TOutput>(Converter<T, TOutput> converter)
		{
			...
			if (count > 0)
			{
				// Convert all items in order. So we start at the cycle's start index. As a bonus, we get a rearranged list.
				var iOutput = 0;
				for (int i = CyclicTailIndex; i < count; i++)
		...
		public T[] ToArray()
		{
			if (CyclicTailIndex < 0) // Means the list is empty.
				return new T[0];
